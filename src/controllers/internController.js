...
const { isValid, isValidEmail, isValidMobile, isValidName } = require("../validation/validation")

//=================== Create Interns For Post API With Validation ====================//
const createIntern = async (req, res) => {
    try {
        ...
        if (!isValidMobile(data.mobile)) {
            return res.status(400).send({status: false, message: "Plz enter valid mobile Number..!!"
            })
        }
        ...
        if (!isValidName(data.collegeName)) {
            return res.status(400).send({status: false, message: "College Name should be string..!!"
            })
        }
